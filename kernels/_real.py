...
class ProductKernel(RealKernel, ProductKernel_):
    ...
    def gradxy(self, X1, X2=None):
        ...
        # with respect to xy.
        grad_xy = sum(Kni[:, :, None, None] * Gxyi for Kni, Gxyi in zip(Kn, Gxy))
        ...
        # overcount.
        grad_x = sum(Gxi * Kni[:, :, None] for Gxi, Kni in zip(Gx, Kn))
        grad_y = sum(Gyi * Ki[:, :, None] for Gyi, Ki in zip(Gy, K))
        grad = grad_xy + grad_x[:, :, :, None] * grad_y[:, :, None, :]

        # get rid of the overcount
        grad -= sum((Kni / Ki)[:, :, None, None] * Gxi[:, :, :, None] * Gyi[:, :, None, :]
                    for Kni, Ki, Gxi, Gyi in zip (Kn, K, Gx, Gy))
